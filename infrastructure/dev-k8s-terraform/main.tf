...
resource "aws_security_group" "petclinic-mutual-sg" {
  ...
  ingress {
    ...
    protocol = "udp"
    from_port = 8472
    to_port = 8472  # flannel portu, cluster kendi içinde iletişim kursun diye
    self = true
  }
    ...
    protocol = "tcp"
    from_port = 2379
    to_port = 2380   # API ile etcd kendi arasında görüşsün diye bu portu açtık.
    self = true
  }
...
resource "aws_instance" "kube-master" {
    ...
    key_name = "clarus"
    subnet_id = "subnet-019156e9a3aa9da07"  # select own subnet_id of us-east-1a
